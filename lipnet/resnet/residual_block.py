...
class BasicBlock1D(tf.keras.layers.Layer):
...
def make_basic_block_layer1D(filter_num, blocks, stride=1):
    res_block = tf.keras.Sequential()
    res_block.add(BasicBlock1D(filter_num, stride=stride))

    for _ in range(1, blocks):
        res_block.add(BasicBlock1D(filter_num, stride=1))

    return res_block

def make_basic_block_layer3D(filter_num, blocks, stride=1):
    res_block = tf.keras.Sequential()
