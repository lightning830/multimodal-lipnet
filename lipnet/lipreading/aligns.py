...
class Align(object):
    def __init__(self, absolute_max_string_len=32, label_func=None):#label_func = [3, 4, 26, 5, 26,,,]のfunc
    ...
    def build(self, align):#align = [(0, 17250, sil), (17250, 22500, bin),,,]
        self.align = self.strip(align, ['sp','sil'])#sp, silのところをとる
        self.sentence = self.get_sentence(align)#'bin blue,,,'
        self.label = self.get_label(self.sentence)#'bin blue'->[3, 2, 5, 26,,]
        self.padded_label = self.get_padded_label(self.label)#[3, 2, 5, 26, ,,-1,-1,-1,] length=32
