...
class Audio(object):
    def __init__(self):
        self.stftname = 'stft0.png'
        self.wavname = 'wav0.wav'

    def wav_read(self, path):
        wave, fs = sf.read(path)# fs = サンプルレート
        return wave, fs

    def from_wav(self, path):
        wave, fs = self.wav_read(os.path.join(path, self.wavname)) 
        wave = wave[:, 0]
        wave = np.array(wave)
    ...
    def from_frames(self, path):
        # print('path', path) #datasets\train\s5\bbie6p
        frames_path = sorted([os.path.join(path, x) for x in os.listdir(path)]) #['datasets\\train\\s13\\lrwy3a\\mouth_000.png', 'datasets\\train\\s13\\lrwy3a\\mouth_001.png',
        # print('frames_path',frames_path)
        frames = [imageio.imread(frame_path) for frame_path in frames_path if self.stftname in frame_path]#(480, 640, 3) ファイル内にstftnameがあれば読み込み
        self.set_data(frames)
        return self

    def set_data(self, frames):
        frame = frames[0] # swap width and height to form format W x H x C
        tmp = frame[:, 0:48, :]
        tmp = tmp[np.newaxis, :, :]
        for i in range(1, 75):
            a = frame[:, i*8:i*8+48, :]
...
class Video(object):
    ...
    def from_frames(self, path):
        # print('path = ', path)
        # print('listdir = ', os.listdir(path)) #listdir(path)pathの中のディレクトリ,ファイルが返る
        frames_path = sorted([os.path.join(path, x) for x in os.listdir(path)])
        frames = [imageio.imread(frame_path) for frame_path in frames_path if 'mouth' in frame_path]#(75, 50, 100, 3) (50, 100, 3)np * 75
        self.handle_type(frames)
        return self


    #動くなら消していい
    # def from_frames_stft(self, path):
    #     # print(path)
    #     frames_path = sorted([os.path.join(path, x) for x in os.listdir(path)])
    #     # print(frames_path)
    #     frames = [imageio.imread(frame_path) for frame_path in frames_path if self.stftname in frame_path]#(480, 640, 3)
    #     self.set_data_stft(frames)
    #     return self

    # def set_data_stft(self, frames):
    #     frame = frames[0] # swap width and height to form format W x H x C
    #     tmp = frame[:, 0:48, :]
    #     tmp = tmp[np.newaxis, :, :]
    #     for i in range(1, 75):
    ...
    #         tmp = np.append(tmp, a, axis = 0)
    #     # print(tmp.shape)
    #     self.data_stft = tmp#(75, 480, 48, 3)

    def from_video(self, path):
        frames = self.get_video_frames(path) #(75, 288, 360, 3)
        # print(frames.shape)
        self.handle_type(frames)
        return self
    ...
    def process_frames_face(self, frames):
        ...
        mouth_frames = self.get_frames_mouth(detector, predictor, frames) #(75, 50, 100, 3)
        pil_img = Image.fromarray(mouth_frames[0])
        # print('now')
        # for i in len(mouth_frames):
        # pil_img.save('tmp/test.jpg')
        # print(detector)
        # print(predictor)
    ...
    def get_frames_mouth(self, detector, predictor, frames):#return list of mouth
        MOUTH_WIDTH = 100
        MOUTH_HEIGHT = 50
        HORIZONTAL_PAD = 0.19
        ...
        for frame in frames:
            ...
            resized_img = np.array(Image.fromarray(frame).resize(new_img_shape, resample=2))
    ...
    def readClip(self, filepass):#ファイルパスを取得しndarrayを返す
    ...
    def get_video_frames(self, path):
        ...
        videogen = self.readClip(path)
        # print('videogen,', videogen)
