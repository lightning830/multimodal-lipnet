...
from lipnet.model3 import LipNet
import numpy as np
import editdistance
import keras
import csv
...
class Statistics(keras.callbacks.Callback):
    ...
    def get_statistics(self, num):
        ...
        while num_left > 0:
            output_batch    = next(self.generator)[0]#get_batchの0番目つまり(input, output)
            num_proc        = min(output_batch['the_input'].shape[0], num_left)#batch,  num_left=256
            y_pred          = self.model_container.predict([output_batch['the_input'][0:num_proc], output_batch['the_input2'][0:num_proc]])
...
stat = Statistics(LipNet, BasicGenerator.next_val, Decoder)
...
#         y_pred       = self.model_container.predict([output_batch['the_input'][0:self.num_display_sentences], output_batch['the_input2'][0:self.num_display_sentences]])
